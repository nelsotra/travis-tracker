        ...
        <b-row>
          ...
          <b-col cols="4"> 
            <b-form-select v-model="current.selectedAdventure">
              <option v-bind:value="{id: o.id, name: o.name, background: o.background, title: o.title, text: o.text, data: o.data}" v-for="o in other">{{ o.name }}</option>
                <hr />
              <optgroup label="Hiking">
                <option v-bind:value="{id: h.id, name: h.name, background: h.background, title: h.title, text: h.text, data: h.data}" v-for="h in hiking">{{ h.name }}</option>
              </optgroup>
                <hr />
              <optgroup label="Biking">
                <option v-bind:value="{id: b.id, name: b.name, background: b.background, title: b.title, text: b.text, data: b.data}" v-for="b in biking">{{ b.name }}</option>
              </optgroup>
                <hr />
              <optgroup label="Regular Ole Touristy Stuff">
                <option v-bind:value="{id: t.id, name: t.name, background: t.background, title: t.title, text: t.text, data: t.data}" v-for="t in tourist">{{ t.name }}</option>
              </optgroup>
            </b-form-select>
          </b-col>
          <b-col cols="4"></b-col>
